...
class TestConnectNiFi:
    ...
    async def test_connect_accepts_auth_alias_and_username_password_aliases(self):
        ...
        data = _parse(result)
        assert data["ok"] is True
        added_conn = mock_registry.add.call_args[0][0]
        assert added_conn.auth_method == "none"
        assert added_conn.knox_user == "user1"
        assert added_conn.knox_password == "pwd1"

    @pytest.mark.asyncio
    async def test_connect_invalid_auth_method_returns_error(self):
        result = await admin.handle(
            "connect_nifi",
            {"name": "a", "url": "https://nifi", "auth_method": "invalid_auth"},
            None,
        )
        data = _parse(result)
        assert "error" in data
        assert "Unsupported auth_method" in data["error"]
...
class TestTestNiFiConnection:
    ...
    async def test_connection_failure(self):
        ...
        assert "error" in data
        assert data["error"] == "Connection test failed"

    @pytest.mark.asyncio
    async def test_test_connection_alias_knox_jwt_maps_to_knox_token(self):
        mock_client = MagicMock()
        mock_client.get_version_info.return_value = {"about": {"version": "2.0.0"}}
        mock_client.session = MagicMock()

        with patch("gateway.tools.admin._build_client", return_value=mock_client) as build_mock:
            result = await admin.handle(
                "test_nifi_connection",
                {"url": "https://nifi/nifi-api", "auth_method": "knox_jwt"},
                None,
            )

        data = _parse(result)
        assert data["ok"] is True
        conn = build_mock.call_args[0][0]
        assert conn.auth_method == "knox_token"
