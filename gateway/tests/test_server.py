...
class TestOAuthEndpoints:
    ...
    def test_oauth_authorization_server_enabled(self):
        with patch("gateway.server.client_manager") as mock_mgr, \
             patch("gateway.server.settings") as mock_settings:
            mock_mgr.get_status.return_value = {}
    ...
    def test_oauth_token_invalid_client_secret_returns_401(self):
        with patch("gateway.server.settings") as mock_settings:
            mock_settings.enable_simple_token_endpoint = True
            mock_settings.api_key = "my-secret-key"
            app = _make_test_app()
    ...
    def test_oauth_token_uses_compare_digest(self):
        calls = []
        ...
        with patch("gateway.server.settings") as mock_settings, \
             patch("gateway.server.hmac.compare_digest", side_effect=fake_compare_digest):
            mock_settings.enable_simple_token_endpoint = True
            mock_settings.api_key = "my-secret-key"
            app = _make_test_app()
            client = TestClient(app, raise_server_exceptions=True)
        ...
        assert resp.status_code == 200
        assert calls == [("my-secret-key", "my-secret-key")]
