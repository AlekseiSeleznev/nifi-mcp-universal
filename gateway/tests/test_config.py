...
class TestSettingsDefaults:
    ...
    def test_persist_secrets_in_state_default_false(self):
        from gateway.config import Settings
        s = Settings()
        assert s.persist_secrets_in_state is False

    def test_simple_token_endpoint_default_false(self):
        from gateway.config import Settings
        s = Settings()
        assert s.enable_simple_token_endpoint is False
...
class TestSettingsEnvVarOverrides:
    ...
    def test_enable_simple_token_endpoint_override(self, monkeypatch):
        monkeypatch.setenv("NIFI_MCP_ENABLE_SIMPLE_TOKEN_ENDPOINT", "true")
        from gateway.config import Settings
        s = Settings()
        assert s.enable_simple_token_endpoint is True

    def test_persist_secrets_override(self, monkeypatch):
        monkeypatch.setenv("NIFI_MCP_PERSIST_SECRETS_IN_STATE", "true")
        from gateway.config import Settings
        s = Settings()
        assert s.persist_secrets_in_state is True
