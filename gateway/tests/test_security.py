...
class TestDashboardApiAuth:
    def test_connect_requires_bearer_when_api_key_enabled(self):
        app = _make_app()
        client = TestClient(app, raise_server_exceptions=False)
        with patch("gateway.web_ui.settings") as ms:
            ms.api_key = "secret-token"
            resp = client.post("/api/connect", json={"name": "a", "url": "https://nifi"})
        assert resp.status_code == 401
        assert resp.json().get("error") == "unauthorized"

    def test_connect_with_bearer_token_is_allowed(self):
        app = _make_app()
        client = TestClient(app, raise_server_exceptions=False)
        conn = ConnectionInfo(name="valid-name", url="https://nifi")
        with patch("gateway.web_ui.settings") as ms, \
            ...
            resp = client.post(
                "/api/connect",
                json={"name": "valid-name", "url": "https://nifi", "auth_method": "none"},
