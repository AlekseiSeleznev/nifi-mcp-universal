...
class TestConnectionInfo:
    ...
    def test_to_dict_drops_sensitive_by_default(self):
        conn = ConnectionInfo(
            name="x",
            url="https://x",
            knox_token="jwt-secret",
            knox_password="pw-secret",
            cert_password="cert-secret",
        )
        with patch("gateway.nifi_registry.settings") as mock_settings:
            mock_settings.persist_secrets_in_state = False
            d = conn.to_dict()
        assert d["knox_token"] == ""
        assert d["knox_password"] == ""
        assert d["cert_password"] == ""

    def test_to_dict_keeps_sensitive_when_enabled(self):
        conn = ConnectionInfo(
            name="x",
            url="https://x",
            knox_token="jwt-secret",
        ...
        with patch("gateway.nifi_registry.settings") as mock_settings:
            mock_settings.persist_secrets_in_state = True
            d = conn.to_dict()
        assert d["knox_token"] == "jwt-secret"
        assert d["knox_password"] == "pw-secret"
        assert d["cert_password"] == "cert-secret"
...
class TestConnectionRegistry:
    ...
    def test_save_uses_atomic_replace(self, tmp_path, monkeypatch):
        ...
        reg = ConnectionRegistry()
        reg._connections["saved"] = ConnectionInfo(name="saved", url="https://s")
        reg._active = "saved"
        ...
        monkeypatch.setattr(reg_mod.os, "replace", fake_replace)
        reg.save()
        assert replace_calls

    def test_save_cleans_up_tmp_file_when_replace_fails(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NIFI_MCP_STATE_FILE", str(tmp_path / "state.json"))
        import gateway.nifi_registry as reg_mod
        monkeypatch.setattr(reg_mod, "STATE_FILE", str(tmp_path / "state.json"))
        ...
        def fake_unlink(path):
            assert path in tmp_files
            raise OSError("already removed")

        monkeypatch.setattr(reg_mod.os, "replace", fake_replace)
        monkeypatch.setattr(reg_mod.os, "unlink", fake_unlink)
        reg.save()
