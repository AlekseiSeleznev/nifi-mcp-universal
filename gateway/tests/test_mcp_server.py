...
class TestCallTool:
    ...
    async def test_exception_in_tool_returns_error_text(self):
        ...
        assert result.isError is True
        assert result.content[0].text == "Error: tool execution failed"
    ...
    @pytest.mark.asyncio
    async def test_known_tool_with_unhandled_module_returns_error(self):
        from gateway.mcp_server import call_tool
        dummy_module = object()
        with patch("gateway.mcp_server._get_session_id", return_value=None), \
             patch("gateway.mcp_server.client_manager") as mock_mgr, \
             patch.dict(mcp_mod._TOOL_DISPATCH, {"mystery_tool": dummy_module}, clear=False):
            mock_mgr.get_client.return_value = MagicMock()
            mock_mgr.get_connection_info.return_value = MagicMock(readonly=True)
            result = await call_tool("mystery_tool", {})

        assert result.isError is True
        assert "Unhandled tool" in result.content[0].text
