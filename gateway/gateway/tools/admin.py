...
from gateway.tools.common import json_text

log = logging.getLogger(__name__)

_AUTH_METHOD_ALIASES = {
    "no_auth": "none",
    "knox_jwt": "knox_token",
}
...
def _resolve_auth_method(arguments: dict) -> str:
    raw = str(arguments.get("auth_method", "none")).strip().lower()
    normalized = _AUTH_METHOD_ALIASES.get(raw, raw)
    if normalized not in _VALID_AUTH_METHODS:
        raise ValueError(
            f"Unsupported auth_method '{raw}'. "
            "Use one of: certificate_p12, certificate_pem, knox_token, knox_cookie, "
            "knox_passcode, basic, none (aliases: no_auth, knox_jwt)."
        )
    return normalized


def _resolve_username(arguments: dict) -> str:
    return str(arguments.get("knox_user") or arguments.get("username", ""))


def _resolve_password(arguments: dict) -> str:
    return str(arguments.get("knox_password") or arguments.get("password", ""))
    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "username": {"type": "string", "description": "Alias for knox_user"},
                "password": {"type": "string", "description": "Alias for knox_password"},
...
def _safe_tool_error(message: str, *, ok: bool | None = None) -> list[TextContent]:
