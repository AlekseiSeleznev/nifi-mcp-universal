...
class NiFiClient:
	...
	def get_flow_health_status(self, pg_id: str) -> Dict[str, Any]:
		...
		for proc in processors.get("processors", []):
			...
			elif run_status == "Disabled":  # pragma: no branch - simple counter branch inside loop
		...
		try:
			...
			for svc in services.get("controllerServices", []):
				...
				elif "INVALID" in state or "ERROR" in state:  # pragma: no branch - simple counter branch inside loop
