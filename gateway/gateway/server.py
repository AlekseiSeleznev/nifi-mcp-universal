...
async def oauth_authorization_server(request: Request) -> JSONResponse:
    ...
    Token endpoint is exposed only in explicit compatibility mode.
    """
    base = str(request.base_url).rstrip("/")
    payload = {"issuer": base}
...
async def oauth_token(request: Request) -> JSONResponse:
    ...
    if not client_secret or not hmac.compare_digest(client_secret, settings.api_key):
        return JSONResponse(
...
async def _run_session(transport: StreamableHTTPServerTransport, ready: asyncio.Event):
    """Pin the transport's mcp_session_id into the task's ContextVar so tool
    handlers can read it. Without this, the ContextVar set by handle_mcp()
    in the HTTP request task is invisible here (this long-lived task was
    created earlier with an empty context) and every call_tool sees
    session_id=None, breaking per-session isolation between concurrent
    clients.
    """
